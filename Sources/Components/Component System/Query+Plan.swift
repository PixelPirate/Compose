...
struct SparseQueryPlan {
    ...
    @usableFromInline
    let others: [UnsafePagedStorage<ContiguousArray.Index>] // entityToComponents maps
    @usableFromInline
    let excluded: [UnsafePagedStorage<ContiguousArray.Index>]
    @usableFromInline
    let version: UInt64
    ...
    init(
        ...
        others: [UnsafePagedStorage<ContiguousArray.Index>],
