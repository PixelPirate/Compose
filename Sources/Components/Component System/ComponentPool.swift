    ...
    func slots<each C: Component>(
                ...
                if excluded.isEmpty {
                    ...
                    return candidates.filter { slot in
                        excludedArrays.allSatisfy { componentArray in
                            !componentArray.entityToComponents.indices.contains(slot.rawValue) || componentArray.entityToComponents[slot.rawValue] == SparseSetInvalidDenseIndex
                        }
                    }
                }
            }
        ...
        for slot in smallest.componentsToEntites {
            ...
            for sparseList in others {
                if sparseList[slot.rawValue] == SparseSetInvalidDenseIndex {
                    presentInAll = false
                    break
                }
            }
            for excluded in excludedArrays where excluded.entityToComponents[slot.rawValue] != SparseSetInvalidDenseIndex {
                presentInAll = false
                break
            }
            if presentInAll {
    ...
    func matches<each C: Component>(slot: SlotIndex, query: Query<repeat each C>) -> Bool {
        for component in repeat (each C).self {
            ...
            guard
                ...
                array.entityToComponents[slot.rawValue] != SparseSetInvalidDenseIndex
            else {
                return false
            }
        }
        ...
        for tag in query.backstageComponents {
            // If any tag is missing or empty, there can be no matches.
            guard let array = self.components[tag], array.entityToComponents[slot.rawValue] != SparseSetInvalidDenseIndex else {
                return false
            }
        }
        ...
        for tag in query.excludedComponents {
            ...
            guard array.entityToComponents[slot.rawValue] == SparseSetInvalidDenseIndex else {
                return false
            }
        }
