...
/// Holds a version counter wired for Observatory observation.
///
/// `ObservationQuery` accesses this bridge through its own `registrar`,
/// mirroring `PerceptibleQuery`'s structure but driven entirely by the
/// self-contained `Observatory` package instead of `Perception`.
@Observatory.Observable
...
/// An observable query that bridges Compose ECS queries to Spacer's UI layer
/// via Observatory.
///
/// `ObservationQuery` owns an internal observation system that runs on the
/// `.perceptionObservation` schedule (after all other schedules). It applies
/// delta updates to a cached result set so observing views only re-render when
...
/// - Observation publication (`registrar.withMutation`) is thread-safe and
...
public final class ObservationQuery<each T: Component>: System, @unchecked Sendable
where repeat each T: ComponentResolving {
    ...
    public init(query: Query<repeat each T>) {
        ...
        self.id = SystemID(name: "ObservationQuery_\(UUID().uuidString)")
    ...
    ///   the cached elements. Safe to iterate in observing views.
